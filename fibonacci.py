...
# num is going to be the integer mentioned in the print statement above;
num = int(input())
print()
...
################### DEFINE GENERATOR FUNCTION
        ...
        # This is a special function called a generator function, it uses
        # 'yield' to generate a SEQUENCE of values instead of returning a 
        # SINGLE value; it returns a generator object, which can then be
        # iterated over using a for loop (among other things);
        def fib_gen(n):
...
######################### DEFINE THE FIBONACCI GENERATOR ITERATOR FUNCTION

        # Define the Fibonacci generator iterator function, which will be used 
        # to iterate over the values generated by our fib_gen function;
        def fib_gen_iter(n):
            
            # i represents each individual value in the Fibonacci sequence,
            # ergo we print the first two values in seq, and then every new 
            # value that is generated;
            for i in fib_gen(n):
                print(i)
            
            # It seemed to be necessary to have some kind of return statement 
            # here, so I used one that returns nothing;
            return ''
        
        # This return statement was also needed, I think to allow this
        # function to pass its values back to the other functions;
        return fib_gen_iter(n)
